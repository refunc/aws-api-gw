...
class LambdaExecutorContainers(LambdaExecutor):
    ...
    def prepare_execution(
        self, func_arn, env_vars, runtime, command, handler, lambda_cwd
    ):
        raise Exception("Not implemented")
    ...
    def execute(
        ...
        if not event:
            LOG.warning(
                'Empty event body specified for invocation of Lambda "%s"' % func_arn
            )
            event = {}
        ...
        cmd = self.prepare_execution(
            func_arn, environment, runtime, command, handler, lambda_cwd
        )
...
class LambdaExecutorReuseContainers(LambdaExecutorContainers):
    ...
    def prepare_execution(
        ...
        container_info = self.prime_docker_container(
            runtime, func_arn, env_vars.items(), lambda_cwd
        )
        ...
        if not has_been_invoked_before:
            ...
            copy_command = 'docker cp "%s/." "%s:/var/task"; ' % (
                lambda_cwd,
                container_info.name,
            )
    ...
    def prime_docker_container(self, runtime, func_arn, env_vars, lambda_cwd):
        ...
        with self.docker_container_lock:
            ...
            if status < 1:
                ...
                LOG.debug(
                    'Copying files to container "%s" from "%s".'
                    % (container_name, lambda_cwd)
                )
                cmd = ("docker cp" ' "%s/." "%s:/var/task"') % (
                    lambda_cwd,
                    container_name,
                )
                LOG.debug(cmd)
    ...
    def destroy_docker_container(self, func_arn):
        ...
        with self.docker_container_lock:
            ...
            if status == 1:
                ...
                cmd = ("docker stop -t0 %s") % (container_name)
    ...
    def get_all_container_names(self):
        ...
        with self.docker_container_lock:
            ...
            cmd = (
                'docker ps -a --filter="name=localstack_lambda_*" --format "{{.Names}}"'
            )
            LOG.debug(cmd)
            cmd_result = run(
                cmd, asynchronous=False, stderr=subprocess.PIPE, outfile=subprocess.PIPE
    ...
    def get_docker_container_network(self, func_arn):
        ...
        with self.docker_container_lock:
            ...
            cmd = ("docker inspect %s" ' --format "{{ .HostConfig.NetworkMode }}"') % (
                container_name
            )
...
class LambdaExecutorSeparateContainers(LambdaExecutorContainers):
    ...
    def get_host_path_for_path_in_docker(self, path):
        return re.sub(
            r"^%s/(.*)$" % config.TMP_FOLDER, r"%s/\1" % config.HOST_TMP_FOLDER, path
        )
